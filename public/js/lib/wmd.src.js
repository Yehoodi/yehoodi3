...
Attacklab.wmdBase = function(){
	...
	// entering links.
	var imageDialogText = "<p style='margin-top: 0px'><b>Enter the image URL.</b></p><p>You can also add a title, which will be displayed as a tool tip.</p><p>Example:<br />http://www.yehoodi.com/images/graphics/forkpath.gif   \"Optional title\"</p>";
	var linkDialogText = "<p style='margin-top: 0px'><b>Enter the web address.</b></p><p>You can also add a title, which will be displayed as a tool tip.</p><p>Example:<br />http://www.yehoodi.com/   \"Optional title\"</p>";
	var youtubeDialogText = "<p style='margin-top: 0px'><b>Enter a YouTube URL.</b></p><p>Example: http://www.youtube.com/watch?v=BU9u9Jb36AU</p>";
	var mp3DialogText = "<p style='margin-top: 0px'><b>Enter a link to an MP3.</b></p><p>Example: http://www.yehoodi.com/shows/LindyManPromo.mp3</p>";
	
	// The default text that appears in the dialog input box when entering
	// links.
	var imageDefaultText = "http://";
	var linkDefaultText = "http://";
	var youtubeDefaultText = "http://";
	var mp3DefaultText = "http://";
	
	// The location of your button images relative to the base directory.
	var imageDirectory = "/images/wmd/wmd-buttons-extra.png";
	...
	var helpLink = "/help/faq#markDown";
	var helpHoverTitle = "Markdown FAQ";
	...
	wmd.PanelCollection = function(){
		...
		this.input = doc.getElementById("wmd_input");
	};
	...
	wmd.editor = function(previewRefreshCallback){
		...
		var makeSpritedButtonRow = function(){
			...
			var youtubeButton = document.createElement("li");
			youtubeButton.className = "wmd-button";
			youtubeButton.id = "wmd-youtube-button";
			youtubeButton.title = "Video";
			youtubeButton.XShift = "-260px";
			youtubeButton.textOp = function(chunk, postProcessing, useDefaultText){
				return command.doYoutube(chunk, postProcessing);
			};
			setupButton(youtubeButton, true);
			buttonRow.appendChild(youtubeButton);

			var mp3Button = document.createElement("li");
			mp3Button.className = "wmd-button";
			mp3Button.id = "wmd-mp3-button";
			mp3Button.title = "Audio";
			mp3Button.XShift = "-280px";
			mp3Button.textOp = function(chunk, postProcessing, useDefaultText){
				return command.doMp3(chunk, postProcessing);
			};
			setupButton(mp3Button, true);
			buttonRow.appendChild(mp3Button);
	...
	command.doYoutube = function(chunk, postProcessing){
		...
		if (chunk.endTag.length > 1) {
			...
			var makeLinkMarkdown = function(link){
				...
				if (link !== null) {
					...
					if (!chunk.selection) {
						chunk.selection = "video title";
					}
				}
				postProcessing();
			};
			
		    util.prompt(youtubeDialogText, youtubeDefaultText, makeLinkMarkdown);
			return true;
		}
	};
	
	command.doMp3 = function(chunk, postProcessing){
		...
		if (chunk.endTag.length > 1) {
			...
			var makeLinkMarkdown = function(link){
				...
				if (link !== null) {
					...
					if (!chunk.selection) {
						chunk.selection = "MP3 title";
					}
				}
				postProcessing();
			};
			
		    util.prompt(mp3DialogText, mp3DefaultText, makeLinkMarkdown);
			return true;
		}
	};
